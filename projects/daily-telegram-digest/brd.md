...
Сообщение отправляется как **plain text**. Скрипт `daily_digest.py` должен удалять или экранировать Markdown-спецсимволы (`*`, `_`, `[`, `]`, `(`, `)`, `~`, `` ` ``, `>`, `#`, `+`, `-`, `=`, `|`, `{`, `}`, `.`, `!`), если они приходят из вывода вложенных скриптов, чтобы избежать ошибок парсинга Telegram и неожиданного форматирования. Заголовок и подвал используют только безопасные символы.
