...
| BR-09 | Система должна поддерживать импорт/экспорт cookies для профилей. | 1) `GET/POST/DELETE /api/cookies/:profileId`; 2) Форматы JSON и Netscape; 3) Инжекция в `user-data-dir` перед стартом; 4) GUI: drag-and-drop + валидатор **(отложено)**. | Should |
| BR-10 | Система должна управлять окнами профилей (раскладка grid/cascade/focus). | 1) Endpoints `/api/window-arranger/*`; 2) Группировка по профилям в GUI **(отложено)**; 3) Cross-platform реализация вместо PowerShell **(Windows-first, macOS/Linux потом)**. | Should |
...
| DD5 | Нет блокирующих открытых вопросов | ✅ Все Q-01..Q-06 закрыты решениями владельца продукта |
| DD6 | Заинтересованные стороны заполнены | ✅ |
| DD7 | Предпосылки для SR заполнены | ✅ Раздел 16 |
| DD8 | **HUMAN GATE** — требуется ревью бизнес-заказчика | ✅ Подтверждено через ответы на открытые вопросы |
...
|---|--------|----------|--------|
|| Q-01 | Подтвердить приоритет Cookie drag-and-drop + валидатора относительно других улучшений. | Владелец продукта | **Закрыт: отложено (не важно сейчас)** |
|| Q-02 | Нужна ли поддержка Window Arranger на macOS/Linux в ближайшем релизе или достаточно Windows-only MVP? | Владелец продукта | **Закрыт: сначала Windows, остальное потом** |
|| Q-03 | Будет ли Migration Wizard и Cloud Sync разморожены? Если да — целевые сроки и приоритет. | Владелец продукта | **Закрыт: будут, но потом** |
|| Q-04 | Требуется ли интеграция с другими кошельками кроме Zerion (MetaMask, Rabby и т.д.)? | Владелец продукта | **Закрыт: пока не требуется, возможно как опция** |
|| Q-05 | Какие регуляторные/юридические требования применимы к хранению cookies/аккаунтов в конкретных юрисдикциях пользователей? | Юрист / Владелец продукта | **Закрыт: не актуально, пользователь хранит свои куки** |
|| Q-06 | Требуется ли SaaS/remote-управление профилями в будущем (влияет на текущую архитектуру `127.0.0.1`)? | Архитектор / Владелец продукта | **Закрыт: не требуется, легко реализуемо потом через HTTPS + keys** |
