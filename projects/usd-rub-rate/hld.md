...
Скрипт `usd_rub_rate.py` — это небольшая консольная утилита на Python 3.11+, работающая исключительно на стандартной библиотеке. При запуске она запрашивает актуальный курс USD/RUB из приоритетного источника (ЦБ РФ), при необходимости переключается на fallback-источник, форматирует результат и выводит его в терминал. В случае невозможности получить данные выводится понятное сообщение об ошибке, процесс завершается с ненулевым кодом возврата.
...
| Parser | Извлечение курса и временной метки из JSON | `json`, `datetime` |
...
### 3.1. Приоритетный источник: неофициальный mirror ЦБ РФ

- **URL:** `https://www.cbr-xml-daily.ru/daily_json.js`
- **Метод:** `GET`
- **Примечание:** Это **неофициальный mirror** курсов ЦБ РФ. Данные предоставляются сторонним сервисом, расположенным за Cloudflare. Для справочного использования допустимо; для критичных финансовых расчётов рекомендуется официальный API ЦБ РФ (`https://www.cbr.ru/scripts/XML_daily.asp`).
- **Преимущества:** не требует API-ключа, отдаёт JSON, стабильно работает для личного использования.
- **Курс:** `Valute.USD.Value`
- **Дата актуальности:** `Date` (ISO-8601, московское время).
...
2. При любой ошибке (HTTP >= 400, таймаут, сеть, некорректный JSON) — переход к fallback.
3. Если и fallback недоступен — вывод ошибки и `exit(1)`.

## 4. Контракт API источников

### 4.1. ЦБ РФ (`daily_json.js`)
...
| `Date` | str | Дата актуальности по московскому времени |
...
| Некорректный JSON от основного источника | Переход к fallback |
| Отсутствует поле `Valute.USD.Value` | Переход к fallback |
...
- Дата приводится к московскому времени (UTC+3) для ЦБ РФ.
- Для fallback-источника UTC-метка конвертируется в MSK.
...
| Требование | Решение |
|------------|---------|
| Stdlib-only | Используются `urllib`, `json`, `argparse`, `datetime`, `sys`, `pathlib`, `tempfile` |
| Размер файла | Целевой размер < 20 КБ, максимум < 50 КБ |
| Время отклика | Общий таймаут не более 10 с с возможностью переопределения |
| Кодировка | UTF-8 для входных и выходных потоков |
...
|---|---------------|-----------|
| 1 | ЦБ РФ может изменить URL или структуру ответа | Версионирование скрипта, unit-тест на парсинг, fallback |
...
| 4 | Часовые пояса | Явное преобразование временных меток в MSK |
...
| T5 | Некорректный JSON от источника | Переход к fallback или ошибка |
| T6 | Проверка формата курса | Ровно два знака после запятой |
| T7 | Проверка часового пояса | Дата в MSK |
