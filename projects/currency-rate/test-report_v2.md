...
| TC-12 | `test_find_previous_day_change_without_today` | Нет записи за сегодня | None | PASS |
| TC-13 | `test_atomic_write_json` | Атомарная запись JSON | target exists, temp deleted | PASS |
| TC-14 | `test_atomic_write_json_concurrent` | Атомарность без остатков | pid совпадает | PASS |
| TC-15 | `test_validate_args_timeout` | Валидация timeout | exit code 2 | PASS |
| TC-16 | `test_validate_args_source_conflict` | Конфликт source/fallback | exit code 2 | PASS |

**Pytest stdout:** `16 passed in 0.10s`.
...
| IT-03 | `report --format text` | `currency_rate.py --timeout 15 report --format text` | 2 строки USD/RUB и EUR/RUB с SMA30 | `USD/RUB: 78.32 (источник: ЦБ РФ, дата: 2026-07-21, SMA30: 78.36)`; EUR аналогично | PASS |
| IT-04 | `report --format json` | `currency_rate.py --timeout 15 report --format json` | Валидный JSON с `rate`/`sma30` округлено до 2 знаков | `rate`: 78.32 / 89.55, `sma30`: 78.36 / 89.71 | PASS |
| IT-05 | `report --format digest` | `currency_rate.py --timeout 15 report --format digest` | Многострочный блок | `USD/RUB: 78.32 (SMA30: 78.36)`\n`EUR/RUB: 89.55 (SMA30: 89.71)` | PASS |
| IT-06 | `history` | `currency_rate.py --timeout 15 --history-days 90 history` | JSON, записи не старше 90 дней, rate округлено | USD rate 78.32, EUR rate 89.55 | PASS |
| IT-07 | `usd_rub_rate.py` legacy wrapper | `python3 usd_rub_rate.py` | Строка `USD/RUB: XX.XX (...)` | `USD/RUB: 78.32 (источник: ЦБ РФ, дата: 2026-07-21, SMA30: 78.36)`, exit=0 | PASS |
...
| IT-17 | Cron schedule presence | `hermes cron list` | Задания `currency-rate-daily-update` (45 7 * * *) и `daily-telegram-digest` (0 8 * * *) присутствуют | Оба задания в списке | PASS |
...
- **Изменение курса за день** в `digest` не отображается на текущих тестовых данных, потому что в истории нет записи за предыдущий календарный день (2026-07-19 — выходной). Функциональность реализована и покрыта unit-тестами TC-08..TC-12.
...
- **Wrapper** использует venv-интерпретатор и strict mode (`set -euo pipefail`).
- **`daily_digest.py`** поддерживает env `PROJECT_DIR` для переносимости.
- **`fetch_all`** теперь параллельный через `ThreadPoolExecutor`.

## Coverage

| Spec item | Covered by | Status |
|---|---|---|
| FR-01 USD/RUB + EUR/RUB | TC-01, TC-02, IT-03..IT-05 | PASS |
| FR-02 Основной источник ЦБ РФ | TC-01, TC-02, IT-02, IT-03 | PASS |
| FR-03 Fallback open.er-api.com | TC-03, IT-11 | PASS |
| FR-04 Кэш с TTL и atomic write | TC-13, TC-14, IT-01 | PASS |
| FR-05 История за 90 дней | TC-05, TC-06, IT-06, IT-18 | PASS |
| FR-06 SMA30 | TC-04, IT-03..IT-05 | PASS |
| FR-07 Изменение курса за день | TC-10, TC-11, TC-12, TC-08, TC-09 | PASS |
| FR-08 Тихое обновление `update` | IT-01, IT-02, IT-08 | PASS |
| FR-09 Подкоманда `report` | IT-03..IT-05, IT-14 | PASS |
...
| SR-03 Валидация аргументов | TC-15, TC-16, IT-12, IT-13 | PASS |
...
| SR-07 Atomic write | TC-13, TC-14 | PASS |
| SR-08 Exit codes | IT-12, IT-13 | PASS |
| SR-09 Форматирование | IT-03..IT-05 | PASS |
| SR-10 Cron интеграция | IT-08, IT-09, IT-16, IT-17 | PASS |
| NFR-01 Производительность | Все вызовы < 15 сек, parallel fetch | PASS |
...
| NFR-05 Портативность | Python 3.11+, venv, PROJECT_DIR env | PASS |

## Verdict

**PASS**

Все 16 unit-тестов и 18 интеграционных сценариев прошли успешно. Критические пути работают корректно. Cron-задания `currency-rate-daily-update` (07:45 MSK) и `daily-telegram-digest` (08:00 MSK) присутствуют в `hermes cron list`.
