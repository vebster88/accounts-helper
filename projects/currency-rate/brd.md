...
- В проекте `currency-rate` лежит скрипт `usd_rub_rate.py`.
...
- Среда разработки: для портативности рекомендуется создавать и использовать **проектный venv** (виртуальное окружение) внутри папки проекта.
- Скрипт должен работать на стандартной библиотеке без `requirements.txt`.
- Целевое развёртывание: ручной запуск пользователем из терминала; опционально — cron/systemd сервера.

## 16а. Среда разработки и развёртывания
...
- Создать venv в папке проекта:
  ...
  ```
- Поскольку скрипт stdlib-only, сторонние зависимости не требуются; `requirements.txt` не обязателен.
- Для форматирования/линтинга (опционально) можно установить `ruff`/`mypy` локально в venv, но эти инструменты не нужны для runtime.

### Production / target runtime

- Целевой хост: сервер с Python 3.11+ (совместимость с системным Python допустима при stdlib-only, но для стабильности рекомендуется venv).
- Способ запуска:
  - Ручной запуск из cron или systemd:
    ```bash
    /home/hermes_ai/my_agent/AI-harness/.venv/bin/python /home/hermes_ai/my_agent/AI-harness/scripts/usd_rub_rate.py
    ```
  - Или `chmod +x scripts/usd_rub_rate.py` и прямой вызов при корректном shebang.
- Конфигурация: CLI-аргументы; секреты и API-ключи не требуются.
