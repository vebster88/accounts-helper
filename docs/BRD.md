...
- PIN состоит ровно из 4 цифр (0–9).
...
- PIN запрашивается повторно только после перезапуска браузера или принудительной блокировки из popup.
...
| BR-08 | Пользователь должен иметь возможность экспортировать и импортировать профиль | Зашифрованный JSON экспортируется и импортируется с PIN | Must |

## Бизнес-правила

| ID | Правило |
|---|---|
| BRULE-01 | PIN должен состоять ровно из 4 цифр (0–9). |
| BRULE-02 | После 5 неправильных попыток ввода PIN профиль блокируется до перезапуска браузера. |
| BRULE-03 | PIN остаётся действительным в течение одной сессии браузера; при закрытии Chrome требуется повторный ввод. |
| BRULE-04 | EVM-адрес должен проходить проверку checksum (EIP-55). |
| BRULE-05 | BTC-адрес должен соответствовать одному из форматов: legacy, P2SH, bech32. |
| BRULE-06 | Email должен соответствовать формату RFC 5322 (упрощённая проверка). |
| BRULE-07 | Discord, Telegram, X handles сохраняются с префиксом `@`, если пользователь его не указал. |
| BRULE-08 | Каждой записи можно назначить псевдоним длиной до 50 символов. |
| BRULE-09 | Только один адрес/запись каждого типа может быть отмечена как «по умолчанию». |
...
| NFR-03 | Безопасность | Ключ шифрования держится только в памяти service worker; удаляется при блокировке или перезапуске браузера |
...
|---|---|---|---|
| 1.0 | 2026-07-22 | Hermes Agent | Первоначальная версия BRD |
| 1.1 | 2026-07-22 | Hermes Agent | Уточнение PIN: 4 цифры, один раз за сессию браузера; BR-08 переведён в Must |
